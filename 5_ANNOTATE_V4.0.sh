...
# soton databases should be in /temp/hgig/GENE_DATABASES/humandb/HG38/ and named as hg38_bloodcancer.txt for $diseaseDB=bloodcancer
####################################################################################################################################
...
table_annovar.pl "$sampleID"_annovar.input /temp/hgig/GENE_DATABASES/humandb/HG38/ -buildver hg38 -out "$sampleID" -remove -protocol refGene,knownGene,avsnp144,1000g2015aug_all,1000g2015aug_afr,1000g2015aug_amr,1000g2015aug_sas,1000g2015aug_eur,1000g2015aug_eas,esp6500siv2_all,esp6500siv2_ea,esp6500siv2_aa,exac03,cosmic70,dbnsfp30a,dbnsfp31a_interpro,dbscsnv11,clinvar_20160302,hrcr1,kaviar_20150923,nci60 -operation g,g,f,f,f,f,f,f,f,f,f,f,f,f,f,f,f,f,f,f,f -arg '-splicing_threshold 20','-splicing_threshold 20',,,,,,,,,,,,,,,,,,, 2>>annovar.log
...
cp /temp/hgig/GENE_DATABASES/humandb/HG38/EXTRA/getseq.c .
...
awk -v smp_no="$sample_no" 'BEGIN{FS=OFS="\t"} NR==FNR{a[$5]=$4;b[$5]=$3;next}{print $0,a[$(132+smp_no)]?a[$(132+smp_no)]"_"b[$(132+smp_no)]:"."}' /temp/hgig/GENE_DATABASES/humandb/HG38/EXTRA/GS2 t4 > t5
...
awk 'BEGIN{FS=OFS="\t"} NR==FNR{a[$1]=$1;next}{print $0,a[$1]?"1":"0"}' /temp/hgig/GENE_DATABASES/humandb/HG38/EXTRA/highly_mutable t6 > t7
# add false positive Genes list from Fuentes et al 2011
awk 'BEGIN{FS=OFS="\t"} NR==FNR{a[$1]=$1;next}{print $0,a[$1]?"1":"0"}' /temp/hgig/GENE_DATABASES/humandb/HG38/EXTRA/fuentes_falsePos t7 > t8
# add ACMG gene list flag
awk 'BEGIN{FS=OFS="\t"} NR==FNR{a[$1]=$1;next}{print $0,a[$1]?"1":"0"}' /temp/hgig/GENE_DATABASES/humandb/HG38/EXTRA/ACMG t8 > t9
...
rm "$sampleID"_annovar.input t1 t2 t3 t4 t5 t6 t7 t8 t9 t10 seqcontext seqcontext_file multianno.key input.key a.out ${sampleID}.hg38_multianno.txt AC AF AN BaseQRankSum ClippingRankSum DP DS END ExcessHet FS HaplotypeScore InbreedingCoeff MLEAC MLEAF MQ MQRankSum QD RAW_MQ ReadPosRankSum SOR SOR_file SOR_short getseq.c getseq
